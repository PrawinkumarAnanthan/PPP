...
c = ufl.variable(c)
f = (6*(1/epsilon)* areainteng)*((c**2)*(1-c)**2)
dfdc = ufl.diff(f, c)
...
F1 = inner(mu, v) * dx - inner(dfdc, v) * dx - 3*epsilon*areainteng*inner(grad(c), grad(v)) * dx- inner(lam1,v)*dx + inner(lam2,v)*dx
...
# # Output file
file = XDMFFile(MPI.COMM_WORLD, "topopt_eqn_withncp_0.5.xdmf", "w")
file.write_mesh(msh)
