...
bottom_facets = mesh.locate_entities_boundary(msh,msh.topology.dim-1,bottom)
bottom_dof = fem.locate_dofs_topological(V.sub(1),msh.topology.dim-1,bottom_facets)
bc_bottom = fem.dirichletbc(default_scalar_type(0),bottom_dof,V.sub(1))
...
right_dof = fem.locate_dofs_topological(V.sub(0),msh.topology.dim-1,right_facets)
bc_right = fem.dirichletbc(default_scalar_type(1),right_dof,V.sub(0))
...
with io.XDMFFile(msh.comm, "stress.xdmf", "w") as xdmf:
    xdmf.write_mesh(msh)
    stress.name = "stress"
